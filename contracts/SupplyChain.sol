...
contract SupplyChain {
    ...
    // Manufacturer;  //Various quality guidelines should be followed by this person
    ...
    //stages of a food product in supply chain
    enum STAGE {
        Init,
        RawMaterialSupply,
        Manufacture,
        ...
        returns (string memory)
    {
        ...
        if (MedicineStock[_productID].stage == STAGE.Init)
            return "Product Ordered";
        else if (MedicineStock[_productID].stage == STAGE.RawMaterialSupply)
            return "Raw Material Supply Stage";
        else if (MedicineStock[_productID].stage == STAGE.Manufacture)
            return "Processing Stage";
        else if (MedicineStock[_productID].stage == STAGE.Distribution)
            return "Distribution Stage";
        else if (MedicineStock[_productID].stage == STAGE.Retail)
            return "Retail Stage";
        else if (MedicineStock[_productID].stage == STAGE.sold)
            return "Product Sold";
    }
    ...
    function addRMS(
        ...
        rmsCtr++;
        RMS[rmsCtr] = rawMaterialSupplier(_address, rmsCtr, _name, _place, _eSewaId);
    }
    ...
    function RMSsupply(uint256 _productID) public {
        require(_productID > 0 && _productID <= medicineCtr);
        uint256 _id = findRMS(msg.sender);
        require(_id > 0);
        require(MedicineStock[_productID].stage == STAGE.Init);
        MedicineStock[_productID].RMSid = _id;
        MedicineStock[_productID].stage = STAGE.RawMaterialSupply;
    }
    ...
    //To process food product
    function Manufacturing(uint256 _productID) public {
        require(_productID > 0 && _productID <= medicineCtr);
        uint256 _id = findMAN(msg.sender);
        require(_id > 0);
        require(MedicineStock[_productID].stage == STAGE.RawMaterialSupply);
        MedicineStock[_productID].MANid = _id;
        MedicineStock[_productID].stage = STAGE.Manufacture;
    }
    ...
    function Distribute(uint256 _productID) public {
        ...
        require(_id > 0);
        require(MedicineStock[_productID].stage == STAGE.Manufacture);
        MedicineStock[_productID].DISid = _id;
        MedicineStock[_productID].stage = STAGE.Distribution;
    }
    ...
    function Retail(uint256 _productID) public {
        ...
        require(_id > 0);
        require(MedicineStock[_productID].stage == STAGE.Distribution);
        MedicineStock[_productID].RETid = _id;
        MedicineStock[_productID].stage = STAGE.Retail;
    }
    ...
    function sold(uint256 _productID) public {
        ...
        uint256 _id = findRET(msg.sender);
        require(_id > 0);
        require(_id == MedicineStock[_productID].RETid); //Only correct retailer can mark product as sold
        require(MedicineStock[_productID].stage == STAGE.Retail);
        MedicineStock[_productID].stage = STAGE.sold;
    }
