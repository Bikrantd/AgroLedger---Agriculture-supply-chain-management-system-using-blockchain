...
function AssignRoles() {
    ...
    const [RMSeSewaId, setRMSeSewaId] = useState(""); // <<< NEW STATE ADDED HERE
    ...
    const handlerChangeeSewaIdRMS = (event) => { // <<< NEW HANDLER ADDED HERE
        setRMSeSewaId(event.target.value);
    }
    ...
    const handlerSubmitRMS = async (event) => {
        ...
        try {
            var receipt = await SupplyChain.methods.addRMS(RMSaddress, RMSname, RMSplace, RMSeSewaId).send({ from: currentaccount }); // <<< UPDATED HERE
            if (receipt) {
                loadBlockchaindata();
            }
        } catch (err) {
            ...
            <div className="row">
                ...
                <div className="col-md-12 mb-3">
                    ...
                    <form onSubmit={handlerSubmitRMS} className="form-inline mb-3">
                        ...
                        <input className="form-control mr-2" type="text" onChange={handlerChangeeSewaIdRMS} placeholder="eSewa ID" required /> {/* <<< NEW INPUT FIELD ADDED HERE */}
            ...
            <table className="table table-bordered">
                <thead className="thead-light">
                    <tr>
                        ...
                        {label === "RMS" && <th>eSewa ID</th>} {/* <<< NEW COLUMN HEADER ADDED HERE */}
                    </tr>
                </thead>
                <tbody>
                    {Object.keys(data).map(key => (
                        <tr key={key}>
                            ...
                            <td>{data[key].addr}</td>
                            {label === "RMS" && <td>{data[key].eSewaId}</td>} {/* <<< NEW COLUMN DATA ADDED HERE */}
                        </tr>
                    ))}
                </tbody>
            </table>
